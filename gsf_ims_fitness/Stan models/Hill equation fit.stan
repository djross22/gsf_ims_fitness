...
parameters {
  real<lower=0., upper=5.3> log_low_level;               // log10 of gene expression level at zero induction
  real<lower=0., upper=5.3>  log_high_level;             // log10 of gene expression level at infinite induction
...
model {
  ...
  target += log1m(erf((0.9 - log_low_level)/0.3));
  target += log1m(erf((log_low_level - 4.9)/0.3));
  //log_high_level ~ normal(3.2, 0.5);
  target += log1m(erf((0.9 - log_high_level)/0.3));
  target += log1m(erf((log_high_level - 4.9)/0.3));
