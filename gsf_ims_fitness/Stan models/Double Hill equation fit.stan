...
parameters {
  ...
  real<lower=log_x_min, upper=log_x_max> log_ec50;           // input level (x) that gives output 1/2 way between g0 and ginf
...
transformed parameters {
  ...
  for (i in 1:N) {
    g[i] = g0 + (ginf - g0)*(x[i]^sensor_n)/(ec50^sensor_n + x[i]^sensor_n);
...
model {
  ...
  // Prior on log_ec50
  target += log1m(erf((log_x_min + 0.7 - log_ec50)/0.5));
  target += log1m(erf((log_ec50 - log_x_max + 0.8)/0.3));
  ...
  // Prior on log_ginf
  target += log1m(erf((log_g_min + 0.9 - log_ginf)/log_g_prior_scale));
  target += log1m(erf((log_ginf - log_g_max + 0.9)/log_g_prior_scale));
