...
data {
  ...
  real<lower=1> nu;      // degrees of freedom for student_t dist
}
...
transformed data {
  ...
  real log_x_min;
  real log_x_max;
  real low_level;         // output at zero input
...
parameters {
  real<upper=0> high_level;         // output at infinite input
...
model {
  ...
  y ~ student_t(nu, mean_y, sigma*y_err);
