...
transformed parameters {
  ...
  logXb = log_ec50 + 1/sensor_n*log10(2^(1/asym) - 1);
  ...
  for (rep in 1:N_reps) {
    for (i in 1:N[rep]) {
	  ...
	  if (x[rep][i]>0) {
        mean_y[rep][i] = g0 + (ginf - g0)/(1 + 1/(x[rep][i]^sensor_n)*10^(sensor_n*logXb))^asym;
      }
      else {
        mean_y[rep][i] = g0;
      }
...
generated quantities {
  ...
  real log_g_out_point;      // log10 output at x_out_point
  
  array[N_reps] vector[N_max] y_resid;
  
  if (x_out_point>0) {
    log_g_out_point = log10(g0 + (ginf - g0)/(1 + 1/(x_out_point^sensor_n)*10^(sensor_n*logXb))^asym);
  }
  else {
    log_g_out_point = log_g0;
  }
  ...
  for (i in 1:30) {
	...
	if (x_out[i]>0) {
      y_out[i] = g0 + (ginf - g0)/(1 + 1/(x_out[i]^sensor_n)*10^(sensor_n*logXb))^asym;
    }
    else {
      y_out[i] = g0;
    }
